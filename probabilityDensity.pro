...
FUNCTION prob2D_21_xi, xi1, xi2
	...
	;;; Compute the linear component of the pdf
	;PRINT,"prob2D_21: kappa_2, lambda_Det = ",kappa_2, lambda_Det
	IF(lambda_Det < 0.) THEN BEGIN
		PRINT,"Negative slope determinant in prob2D_21_xi() , quitting..."
		RETALL
	ENDIF
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; Function to compute derivative w.r.t. kappa_xi_21 of 2D slope probability 
;;; density function for the case where lambda_101 goes to unity

FUNCTION prob2D_21_xi_diff_21, xi1, xi2
	...
	;;; Compute the linear component of the pdf
	;PRINT,"prob2D_12: kappa_2, lambda_Det = ",kappa_2, lambda_Det
	IF(lambda_Det < 0.) THEN BEGIN
		PRINT,"Negative slope determinant in prob2D_21_xi_diff_21() , quitting..."
		RETALL
	ENDIF
	...
	;;; Add the nonlinear correction
	probDens = (1.D/6.D)*probDensLin*(kappa_2^(-1.5D))*(9.D*He2_21+3.D*He2_12)
...
FUNCTION prob2D_21_xi_diff_21_int, xi1, xi2

	prob_xi_Return = DBLARR(N_ELEMENTS(xi2))
	FOR i=0,N_ELEMENTS(xi2)-1 DO prob_xi_Return[i]=prob2D_21_xi_diff_21(xi1,xi2[i])
	RETURN,prob_xi_Return
END
...
FUNCTION prob2D_21_xi_diff_12, xi1, xi2
	...
	;PRINT,"prob2D_12: kappa_2, lambda_Det = ",kappa_2, lambda_Det
	IF(lambda_Det < 0.) THEN BEGIN
		PRINT,"Negative slope determinant in prob2D_21_xi_diff_12() , quitting..."
		RETALL
	ENDIF
	...
	;;; Add the nonlinear correction
	probDens = (1.D/6.D)*probDensLin*(kappa_2^(-1.5D))*(3.D*He2_12+3.D*He2_21)
...
FUNCTION prob2D_21_xi_diff_12_int, xi1, xi2

	prob_xi_Return = DBLARR(N_ELEMENTS(xi2))
	FOR i=0,N_ELEMENTS(xi2)-1 DO prob_xi_Return[i]=prob2D_21_xi_diff_12(xi1,xi2[i])
	RETURN,prob_xi_Return
END
...
FUNCTION prob2D_12_xi, xi1, xi2
	...
	;PRINT,"prob2D_12: kappa_2, lambda_Det = ",kappa_2, lambda_Det
	IF(lambda_Det < 0.) THEN BEGIN
		PRINT,"Negative slope determinant in prob2D_12_xi() , quitting..."
		RETALL
	ENDIF
...
FUNCTION prob2D_12_xi_diff_21, xi1, xi2
	...
	;PRINT,"prob2D_12: kappa_2, lambda_Det = ",kappa_2, lambda_Det
	IF(lambda_Det < 0.) THEN BEGIN
		PRINT,"Negative slope determinant in prob2D_12_xi_diff_21() , quitting..."
		RETALL
	ENDIF
	...
	;;; Add the nonlinear correction
	probDens = (1.D/6.D)*probDensLin*(kappa_2^(-1.5D))*6.D*He2_21
...
FUNCTION prob2D_12_xi_diff_21_int, xi1, xi2

	prob_xi_Return = DBLARR(N_ELEMENTS(xi2))
	FOR i=0,N_ELEMENTS(xi2)-1 DO prob_xi_Return[i]=prob2D_12_xi_diff_21(xi1,xi2[i])
	RETURN,prob_xi_Return
END
...
FUNCTION prob2D_12_xi_diff_12, xi1, xi2
	...
	;PRINT,"prob2D_21: kappa_2, lambda_Det = ",kappa_2, lambda_Det
	IF(lambda_Det < 0.) THEN BEGIN
		PRINT,"Negative slope determinant in prob2D_12_xi_diff_12() , quitting..."
		RETALL
	ENDIF
	probDensLin = EXP(-0.5D*quadForm)/(2.D*!DPI*kappa_2*SQRT(lambda_Det))

	;;; Add the nonlinear correction
	probDens = (1.D/6.D)*probDensLin*(kappa_2^(-1.5D))*12.D*He2_12
...
FUNCTION prob2D_12_xi_diff_12_int, xi1, xi2

	prob_xi_Return = DBLARR(N_ELEMENTS(xi2))
	FOR i=0,N_ELEMENTS(xi2)-1 DO prob_xi_Return[i]=prob2D_12_xi_diff_12(xi1,xi2[i])
	RETURN,prob_xi_Return
END
...
;;; Function to compute the derivative of the 3D slope probability density function
;;; with respect to lambda_xi_111

FUNCTION prob3D_xi_diff_111, xi1, xi2, xi3
	...
	;;; Add the nonlinear correction
	probDens = probDensLin*(kappa_2^(-1.5D))*He3_111
...
FUNCTION prob3D_xi_diff_111_int, xi1, xi2, xi3
	prob_xi_Return = DBLARR(N_ELEMENTS(xi3))
	FOR i=0,N_ELEMENTS(xi3)-1 DO prob_xi_Return[i]=prob3D_xi_diff_111(xi1,xi2,xi3[i])
	RETURN,prob_xi_Return
END
