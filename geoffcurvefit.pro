...
;
;-
FUNCTION GEOFFCURVEFIT, x, y, weightsIn, a, sigma, FUNCTION_NAME = Function_Name, $
    FITA=fita, $
...
;            Evaluate function and estimate partial derivatives
	      ...
	      PRINT,"iparam = ",iparam
		  PRINT,"pder[*,iparam] = ",pder[*,iparam]
		  PRINT,"TRANSPOSE(pder[*,iparam]) = ",TRANSPOSE(pder[*,iparam])
		  PRINT,"Weights = ",Weights
		  PRINT,"(FLTARR(nparam)+1) = ",(FLTARR(nparam)+1)
		  PRINT,"(FLTARR(nparam)+1)*pder = ",(FLTARR(nparam)+1)*pder
		  PRINT,"(Weights # (FLTARR(nparam)+1)*pder) = ",(Weights # (FLTARR(nparam)+1)*pder)
          ...
          REPEAT BEGIN
			 ...
			 PRINT,"chisq, chisq1 = ",chisq,chisq1
